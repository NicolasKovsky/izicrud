...
const props = defineProps<{
    itens: {
        data: { id: number; nome: string; nivel: number; descricao: string; ativo: boolean }[];
        current_page: number;
        last_page: number;
        per_page: number;
        total: number;
    };
    allItens: { id: number; nome: string; nivel: number; descricao: string; ativo: boolean }[];
    ...
    <AppLayout :breadcrumbs="breadcrumbs" :headerTitle="headerTitle" :headerDescription="headerDescription" :sidebarNavItems="props.sidebarNavItems">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            ...
            <div class="relative min-h-[100vh] flex-1 rounded-xl border border-sidebar-border/70 dark:border-sidebar-border md:min-h-min">
                <div class="mb-4">
                    <input v-model="searchQuery" type="text" placeholder="Pesquisar permissoes..." class="w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>

                <Table>
                    <TableHeader>
                        <TableRow>
                            <TableHead class="cursor-pointer" @click="toggleSort('nome')">Nome do Grupo<span v-if="sortColumn === 'nome'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('nivel')">Nivel de Acesso<span v-if="sortColumn === 'nivel'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('descricao')">Descrição do Grupo<span v-if="sortColumn === 'descricao'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('ativo')">Está ativo?<span v-if="sortColumn === 'ativo'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                        ...
                        <TableRow v-for="(item, index) in paginatedItens" :key="index">
                            ...
                            <TableCell>
                                <ul class="flex flex-row gap-4">
                                    <li>
                                        <Button variant="link" @click="router.visit(`/permissoes/${item.id}/edit`)" class="text-blue-500 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 ring-1 ring-blue-500/20">
